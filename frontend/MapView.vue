...
const openPopulationDistribution = async () => {
  ...
  setTimeout(async () => {
    try {
      ...
      // 并行加载最多2个shapefile的字段，每个请求5秒超时
      const maxFiles = Math.min(shapefiles.length, 2)
      ...
      if (populationFieldOptions.value.length > 0) {
        ...
        console.log('字段加载完成，默认选择:', defaultField)
      } else {
    ...
    setTimeout(() => {
      ...
      if (closeBtn) {
        closeBtn.onclick = (event) => {
          ...
          // 确保商圈人口分布对话框关闭
          circleDialogVisible.value = false
          console.log('关闭按钮点击，移除统计面板及半径圆、网格')
          ...
          if (populationLayerGroup && map.hasLayer(populationLayerGroup)) {
            ...
            layers.forEach(layer => {
              // 检查是否为标记图层（圆心标记）
              if (layer instanceof L.Marker || (layer.getLatLng && typeof layer.getLatLng === 'function')) {
                // 区分圆心标记和统计面板标记：圆心标记使用默认图标或图钉图标，统计面板标记使用divIcon
                const icon = layer.options.icon
                if (icon && icon.options && icon.options.html) {
                  // 这是统计面板标记（divIcon），跳过
                  return
                }
                // 如果已经找到一个标记，优先选择不是统计面板的标记
                if (!centerMarker) {
                  centerMarker = layer
                }
              }
            ...
            if (centerMarker) {
              ...
              // 绑定点击事件：点击时重新显示半径圆、网格和统计面板
              centerMarker.off('click') // 移除之前的点击事件
              centerMarker.on('click', () => {
                console.log('圆心标记被点击，重新显示人口分布图层')
                // 将圆心标记添加回图层组
                if (populationLayerGroup) {
                  populationLayerGroup.addLayer(centerMarker)
                }
                // 将图层组添加到地图（显示半径圆、网格和统计面板）
                if (populationLayerGroup && !map.hasLayer(populationLayerGroup)) {
