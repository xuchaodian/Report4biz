...
const startPopulationCompare = async () => {
  ...
  try {
    ...
    // 构建城市到shapefile的映射（根据文件名包含的城市名）
    const cityToShapefileMap = {}
    for (const sf of shapefiles) {
      if (!sf.name) continue
      // 提取城市名：去除文件扩展名和数字等，保留中文字符
      const cityMatch = sf.name.match(/^([\u4e00-\u9fa5]+)/)
      if (cityMatch) {
        const cityName = cityMatch[1]
        cityToShapefileMap[cityName] = sf
        console.log(`城市映射: ${cityName} -> ${sf.name}`)
      }
    }
    console.log('城市映射表:', Object.keys(cityToShapefileMap))
    console.log('shapefiles列表:', shapefiles.map(sf => ({ name: sf.name, id: sf.id })))
    ...
    let statField = null
    let allIntegerFields = []
    if (geojson?.features?.length > 0) {
      const props = geojson.features[0].properties || {}
      console.log('=== 字段分析开始 ===')
      console.log('第一个feature的所有属性:', props)
      console.log('features数量:', geojson.features.length)
      
      // 首先检查是否有"pop_total"或"人口"相关字段
      const populationFieldCandidates = ['pop_total', 'population', '常住人口', '人口', '总人口', 'total_population']
      ...
      for (const [key, val] of Object.entries(props)) {
        if (key !== 'RecID') {
          ...
          const isNumber = !isNaN(numVal) && isFinite(val)
          
          if (isNumber) {
            allIntegerFields.push({key, value: val, numericValue: numVal, type: typeof val})
            ...
            // 检查是否是候选人口字段
            if (populationFieldCandidates.some(candidate => key.toLowerCase().includes(candidate.toLowerCase()))) {
              if (!candidateFound) {
                statField = key
                candidateFound = true
                console.log(`🔍 发现候选人口字段: ${key} = ${val}`)
              }
            }
          } else {
            console.log(`字段 '${key}' 不是数字:`, val, `类型: ${typeof val}`)
          }
        }
      }
      
      // 如果没有找到任何数字字段，尝试使用第一个非RecID字段
      if (allIntegerFields.length === 0) {
        for (const [key, val] of Object.entries(props)) {
          if (key !== 'RecID') {
            statField = key
            console.log(`⚠️ 没有数字字段，使用第一个非RecID字段: ${key}`)
            break
          }
        }
      }
      ...
      // 详细输出找到的数字字段
      console.log('找到的数字字段 (共' + allIntegerFields.length + '个):')
      allIntegerFields.forEach((field, idx) => {
        console.log(`  ${idx+1}. ${field.key} = ${field.value} (类型: ${field.type}, 数值: ${field.numericValue})`)
      })
      console.log('选定的统计字段:', statField)
      console.log('=== 字段分析结束 ===')
    } else {
      console.log('=== 字段分析失败 ===')
    ...
    const extractCityFromStore = (store) => {
      console.log(`提取门店城市:`, store)
      // 1. 直接使用city字段
      if (store.city && typeof store.city === 'string' && store.city.trim()) {
        const city = store.city.trim()
        console.log(`  使用city字段: "${city}"`)
        return city
      }
      // 2. 从address中提取（如果有）
      if (store.address && typeof store.address === 'string') {
        ...
        console.log(`  从address提取: "${address}"`)
        // 尝试匹配常见城市名（如上海、北京等）
        const cityMatch = address.match(/上海|北京|广州|深圳|杭州|南京|成都|重庆|武汉|西安|苏州|天津/)
        if (cityMatch) {
          console.log(`  地址匹配到城市: "${cityMatch[0]}"`)
          return cityMatch[0]
        }
      }
      // 3. 从name中提取（如果有）
      if (store.name && typeof store.name === 'string') {
        ...
        console.log(`  从name提取: "${name}"`)
        const cityMatch = name.match(/上海|北京|广州|深圳|杭州|南京|成都|重庆|武汉|西安|苏州|天津/)
        if (cityMatch) {
          console.log(`  名称匹配到城市: "${cityMatch[0]}"`)
          return cityMatch[0]
        }
      }
      // 4. 无法提取
    ...
    for (const store of storesToCompare) {
      ...
      console.log(`门店 "${store.name}" 城市提取结果:`, storeCity)
      
      // 为当前门店创建定制的shapefile获取函数
      const getShapefilesForStore = async () => {
        console.log(`=== getShapefilesForStore 被调用，门店城市: ${storeCity} ===`)
        const results = []
        
        // 如果找到城市名，使用API的城市过滤参数获取对应城市的shapefile
        if (storeCity) {
          const encodedCity = encodeURIComponent(storeCity)
          ...
          try {
            ...
            if (!listRes.ok) {
              ...
              const listData = await listRes.json()
              const filteredShapefiles = Array.isArray(listData) ? listData : (listData.data || [])
              console.log(`城市过滤返回 ${filteredShapefiles.length} 个shapefile:`, filteredShapefiles.map(sf => ({ id: sf.id, name: sf.name, city: sf.city })))
              
              if (filteredShapefiles.length > 0) {
                // 加载每个shapefile的完整GeoJSON数据
                for (const sf of filteredShapefiles) {
                  try {
                    ...
                    if (geojson) {
                      ...
                      console.log(`成功加载城市对应的shapefile: ${sf.name}`)
                    } else {
                      console.warn(`shapefile无geojson数据: ${sf.name}`)
                    }
                  } catch (e) {
                    console.error(`获取shapefile ${sf.id} 失败:`, e)
                  }
                }
              } else {
                console.warn(`城市过滤未找到匹配的shapefile，将回退到所有shapefile`)
              }
            }
          } catch (e) {
            console.error(`城市过滤API调用异常:`, e)
          }
        }
        
        // 如果没有城市名、城市过滤未找到数据或API调用失败，回退到加载所有shapefile
        if (results.length === 0) {
          console.log(`回退到加载所有shapefile (共${shapefiles.length}个)`)
          for (const sf of shapefiles) {
            if (!sf) continue
            try {
              const res = await fetch(`/api/shapefiles/${sf.id}`, {
              ...
              console.error(`获取shapefile失败:`, e)
            }
          }
        }
        
        console.log(`getShapefilesForStore 返回 ${results.length} 个结果`)
        return results
      }

      console.log(`=== 准备调用 calculatePopulationByRadius ===`)
      console.log(`传递的 getShapefilesForStore 函数:`, typeof getShapefilesForStore)
      console.log(`门店城市: ${storeCity}`)
      const result = await calculatePopulationByRadius(lat, lng, radiusMeters, statField, getShapefilesForStore)
      allResults.push({
        ...store,
        total: result.total,
        statField,
